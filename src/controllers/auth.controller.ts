...
export async function createMember(req: Request, res: Response) {
  try {
    ...
    const { username, password, lastName, firstName } = dataInput;

    if (username && password && lastName && firstName) {
      try {
        const hashPws = await hashPassword(password);
        const [response, created] = await handleCreateMember({ ...dataInput, password: hashPws });
  ...
  try {
    ...
    if (!username || !password) {
      ...
      try {
        ...
        // * Create access token
        const accessToken = createToken(member, ACCESS_TOKEN_PRIVATE_KEY, accessTokenTtl);

        // * create refresh token
        const refreshToken = createToken(member, REFRESH_TOKEN_PRIVATE_KEY, refreshTokenTtl);
        ...
        res.send({ message: 'Logged in Success', error: false, accessToken });
      } catch (error) {
        if (error instanceof Error) {
          log.error(error.message);
          return res.status(400).json({ message: error.message, error: error.name });
...
export async function refreshToken(req: Request, res: Response) {
  try {
    ...
    // * GET REFRESH TOKEN FROM DATABASE => SESSION
    const session = await db.Session.findOne({ where: { token: refreshToken } });

    if (!session)
      return res.status(403).json({ error: true, message: 'Refresh token is not valid' });
    ...
    const newAccessToken = createToken(hashJti, ACCESS_TOKEN_PRIVATE_KEY, accessTokenTtl);
    const newRefreshToken = createToken(hashJti, REFRESH_TOKEN_PRIVATE_KEY, refreshTokenTtl);
