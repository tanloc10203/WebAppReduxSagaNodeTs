...
import { QueryOptionsWithWhere } from 'sequelize/types';
...
export function findMember(query: QueryOptionsWithWhere) {
  return db.Member.findOne(query);
}
...
export function createToken(user: MemberAttribute, privateKey: string, expiresIn: string) {
  const { password, ...others } = user;

  const jti: string = signJti(others);
